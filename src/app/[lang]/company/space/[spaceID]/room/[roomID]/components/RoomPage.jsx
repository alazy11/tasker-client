...
import Running from "@/app/[lang]/user/space/[spaceID]/room/[roomID]/components/Running";
...
function getUserInformation(id,setUsers,userStream) {
    ...
    fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/en/company/getUserInformation?user=${id}`, {
        credentials: "include",
        headers: {
...
export default function RoomPage({user,spaceID,meetingID,creator}) {
    ...
    useEffect(() => {
        ...
        fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/en/company/space/${spaceID}`, {
            ...
            .then((data) => {
                if (data.status === "fail" || data.status === "error") {
                    ...
                    navigator.mediaDevices.getUserMedia({
                        ...
                        console.log("company peer",user.room_ID)
                        setMyPeer(new Peer('company'+user.room_ID))
                    }).catch((err)=>{
                        console.log(err)
                    })
            ...
            fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/en/company/space/${spaceID}/meeting/${meetingID}`, {
                signal: abortController.signal,
                method: 'PUT',
                credentials: "include",
                headers: {
    ...
    return (
        <div className="h-full w-full fixed top-0 left-0 cont-meet ">
            <a href={`/en/company/space/${spaceID}`} ref={link} className="hidden" ></a>
...

{
...

                {
                    users.map((item, index)=>{
                        return(
                            <VideoElement key={item.public_name + index} roomId={roomId} user={item} stream={item.stream} />
                        )
                    })
                }
