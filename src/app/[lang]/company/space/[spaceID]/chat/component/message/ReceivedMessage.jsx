...
function handelDownload(spaceID, filePath, fileName) {
   ...
   url = `${process.env.NEXT_PUBLIC_BACKEND_URL}/en/company/space/${spaceID}/chat/download?filePath=${filePath}&fileName=${fileName}`;
   // } else {
...
export default function ReceivedMessage({
   ...
   return (
      <div
         ...
         className={`w-full ${message.company_id === user.company_id
            ? "send-message"
            : "incoming-message"
            }`}
      >
