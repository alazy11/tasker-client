...
export default function AddMember({ setEmployee,company, spaceID, referesh,setReferesh }) {
         ...
         `http://localhost:4040/en/company/employee?page=${page}&recordNumber=${recordNumber}`,
       {
         ...
         .then((data) => {
            if (data.status === "fail" || data.status === "error") {
               ...
               console.log("employee user.....", [...Object.values(data.data.result)]);
               setUser([...Object.values(data.data.result)]);
            }
         })
         .catch((error) => {
            console.log(error);
         ...
         <div className="flex flex-col h-full">
            ...
            <div className="w-full overflow-auto flex-1 scroll-bar pb-2.5 h-5/6">
               <ul className="flex flex-col gap-1">
                  {user?.map((item) => {
                     return (
                        <li
                           ...
                           onClick={(e)=>{
                              setModel(true);
                              setEmployeeUserName(e.currentTarget.dataset.user);
                              setEmployeeID(e.currentTarget.dataset.userid);
                              console.log("e.currentTarget.dataset.userID",e.currentTarget.dataset.userid)
                           }}
                           key={item.user_id}
                        >
                  ...
                  {
                     model && <EmployeeInformation referesh={referesh} setReferesh={setReferesh} spaceID={spaceID} company={company} employeeUserName={employeeUserName} setModel={setModel} employeeID={employeeID} setNotification={setNotification} setMessage={setMessage} />
                  }
