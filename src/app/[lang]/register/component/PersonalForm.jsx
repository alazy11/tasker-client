...
function formHandler(e,cleanf,setErrorMessage,setErrorText,setLoader,router,...data) {
   ...
   // clean(...cleanf);
   fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/en/register`,{
      method:"post",
      credentials: 'include',
      headers:{
...
async function searchByUserName(e,user,setError,setDisable) {
   if(user.param === 'userName') {
      fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/en/register?userName=${user.data}`,{
      headers:{
         "content-type":"application/json"
      }
   }).then(res=>{
      ...
      fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/en/register?email=${user.data}`,{
      headers:{
         "content-type":"application/json"
      }
   }).then(res=>{
