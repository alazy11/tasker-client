...
function clean(...fun) {
   fun.forEach((setfun,ind)=>{
      setfun(`${process.env.NEXT_PUBLIC_BACKEND_URL}/en/register/company`);
   });
}
...
function formHandler(e,cleanf,setErrorMessage,setErrorText,next,setNext,setViewNext,disabledPerson,setCommingSecretKey,commingSecretKey,inputSecretKey,setDisable, router,...data) {
   ...
   if(next === '') {
      ...
      setViewNext(true);
      disabledPerson(true);
      fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/en/register/company/getSecretKey?companyName=${data[0]}&companyEmail=${data[3]}`).then(res=>{
         return res.json();
      }).then(data=>{
...
async function searchByUserName(e,user,setError,setDisable) {
   if(user.param === 'userName') {
      fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/en/register/company?userName=${user.data}`,{
      headers:{
         "content-type":"application/json",
         "cache-control":"no-cache",
      }
      ...
      fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/en/register/company?email=${user.data}`,{
      headers:{
         "content-type":"application/json",
         "cache-control":"no-cache",
      }
...
export default function PersonalForm({dic, disabledPerson}) {
  ...
  <path d="M16.978 19.502a9 9 0 1 1 4.022-7.5c0 2.072-.75 3.75-2.625 3.75s-2.625-1.678-2.625-3.75v-3.75"></path>
         ...
         <div className="form-group  relative">
            ...
            <div className="field-group">
               <span>
               <svg width="22" height="22" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="h-5 w-5">
